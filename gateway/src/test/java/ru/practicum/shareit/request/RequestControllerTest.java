...
public class RequestControllerTest {
    ...
    private ResponseEntity<Object> requestResponse;
    ...
    void beforeEach() {
        ...
        requestResponse = new ResponseEntity<>(TestConvert.asJsonString(requestDto), HttpStatus.OK);
    }

    @Test
    void testCreate() throws Exception {
        when(requestClient.create(anyInt(), any())).thenReturn(requestResponse);
    ...
    void testCreateUserNotFound() throws Exception {
        requestResponse = new ResponseEntity<>(Map.of("message", "нет пользователя с id 1"), HttpStatus.NOT_FOUND);
        when(requestClient.create(anyInt(), any())).thenReturn(requestResponse);
    ...
    void testGetAll() throws Exception {
        requestResponse = new ResponseEntity<>(TestConvert.asJsonString(List.of(requestAnswerDto)), HttpStatus.OK);
        when(requestClient.getAll(anyInt())).thenReturn(requestResponse);
    ...
    void testGetRequestsForUserOwnedItems() throws Exception {
        requestResponse = new ResponseEntity<>(TestConvert.asJsonString(List.of(requestAnswerDto)), HttpStatus.OK);
        when(requestClient.getOtherUsersRequests(anyInt(), anyInt(), anyInt())).thenReturn(requestResponse);
    ...
    void testGetRequestByIdFull() throws Exception {
        requestResponse = new ResponseEntity<>(TestConvert.asJsonString(requestAnswerDto), HttpStatus.OK);
        when(requestClient.getRequestByIdFull(anyInt(), anyInt())).thenReturn(requestResponse);
